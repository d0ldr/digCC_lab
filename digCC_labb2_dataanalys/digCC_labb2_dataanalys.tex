...
%Now trying \lstinline[language=Python]{print(''Hello Void!'')} to see how that rocks!
...
\begin{enumerate}
  ...
  \begin{lstlisting}[language=Python]
    ...
    \end{lstlisting}
%Här kallar vi den inlästa data (är en variabel på Pythonspråk) \verb+stegdata+. \lstinline[language=Python]{sep = ”;”} anger att det är semikolon som separerar kolumner från varandra. Kör du denna cell laddas stegdatan in och du kan komma åt den genom att använda variabeln \verb+stegdata+. 
...
plt.figure(dpi=100) # mer DPI ger snyggare bild
plt.scatter(X, y) # X och y som punkter
plt.plot(X, y_predict1, 'r') # X och y-s appriximering som linje
plt.xlabel("Year") # text pa x-axeln
plt.ylabel("Temperatur (grader C)") # text pa y-axeln
plt.legend(["Data","Model" ])  # legend till kurvorna
plt.show() # kravs inte i Jupyter, Python-standard
\end{lstlisting}
